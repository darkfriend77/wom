...
namespace WoMInterface.Tool
{
    class CommandLine
    {
        ...
        private void Print(Mogwai mogwai)
        {
            ...
            @"+-stats----------+-------------------+-body--------------+",
            @"¦ STR:   <str  > ¦ GENDER: <gen    > ¦ SKINCOL:  <skinc> ¦".Replace("<str  >", string.Format("{0,7}",mogwai.Abilities.Strength)).Replace("<gen    >", string.Format("{0,9}",mogwai.Body.MapGender())).Replace("<skinc>", string.Format("{0,7}",mogwai.Body.SkinColor)),
            @"¦ CON:   <con  > +-state-------------+ EYECOL:   <eyec > |".Replace("<con  >", string.Format("{0,7}",mogwai.Abilities.Constitution)).Replace("<eyec >", string.Format("{0,7}",mogwai.Body.EyeColor)),
            @"¦ DEX:   <dex  > ¦ HP:   <hp       > ¦ EAR:      <ear  > ¦".Replace("<dex  >", string.Format("{0,7}",mogwai.Abilities.Dexterity)).Replace("<ear  >", string.Format("{0,7}",mogwai.Body.Ear)),
            @"¦ INT:   <int  > ¦ MANA: <mana     > ¦ MOUTH:    <mouth> ¦".Replace("<int  >", string.Format("{0,7}",mogwai.Abilities.Inteligence)).Replace("<mouth>", string.Format("{0,7}",mogwai.Body.Mouth)),
            @"¦ WIS:   <wis  > ¦ END:  <end      > +-coat--------------+".Replace("<wis  >", string.Format("{0,7}",mogwai.Abilities.Wisdom)),
            @"¦ CHA:   <cha  > +-------------------+ COATTYPE: <ctype> ¦".Replace("<cha  >", string.Format("{0,7}",mogwai.Abilities.Charisma)).Replace("<ctype>", string.Format("{0,7}",mogwai.Coat.CoatType)),
