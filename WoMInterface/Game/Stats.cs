...
namespace WoMInterface.Game
{
    public class Stats
    {
        public Attribute StrengthAttr = AttributBuilder.Create("Strength")
            .Salted(true).Position(30).Size(2).Creation(32).MinRange(1).MaxRange(256)
            .Description("A measure of how physically strong a character is. Strength often controls the maximum weight the character can carry, melee attack and/or damage, and sometimes hit points.Armor and weapons might also have a Strength requirement.").Build();
        public int Strength => StrengthAttr.GetValue();

        public Attribute ConstitutionAttr = AttributBuilder.Create("Constitution")
            .Salted(true).Position(32).Size(2).Creation(32).MinRange(1).MaxRange(256)
            .Description("A measure of how sturdy a character is. Constitution often influences hit points, resistances for special types of damage(poisons, illness, heat etc.) and fatigue.").Build();
        public int Constitution => ConstitutionAttr.GetValue();

        public Attribute DexterityAttr = AttributBuilder.Create("Dexterity")
            .Salted(true).Position(34).Size(2).Creation(32).MinRange(1).MaxRange(256)
            .Description("A measure of how agile a character is. Dexterity controls attack and movement speed and accuracy, as well as evading an opponent's attack (see Armor Class).").Build();
        public int Dexterity => DexterityAttr.GetValue();

        public Attribute InteligenceAttr = AttributBuilder.Create("Intelligence")
            .Salted(true).Position(36).Size(2).Creation(32).MinRange(1).MaxRange(256)
        ...
        public Attribute WisdomAttr = AttributBuilder.Create("Wisdom")
            .Salted(true).Position(38).Size(2).Creation(32).MinRange(1).MaxRange(256)
            .Description("A measure of a character's common sense and/or spirituality. Wisdom often controls a character's ability to cast certain spells, communicate to mystical entities, or discern other characters' motives or feelings.").Build();
        public int Wisdom => WisdomAttr.GetValue();

        public Attribute CharismaAttr = AttributBuilder.Create("Charisma")
            .Salted(true).Position(40).Size(2).Creation(32).MinRange(1).MaxRange(256)
            .Description("A measure of a character's social skills, and sometimes their physical appearance. Charisma generally influences prices while trading and NPC reactions. Under certain circumstances, this skill can negate combat actions between players and NPC enemies.").Build();
        ...
        public Stats(HexValue hexValue, Shift shift)
        {
            // new creation
            Dice dice = new Dice(shift);

            StrengthAttr.RollValue(dice);
            ConstitutionAttr.RollValue(dice);
            DexterityAttr.RollValue(dice);
            InteligenceAttr.RollValue(dice);
            WisdomAttr.RollValue(dice);
            CharismaAttr.RollValue(dice);

            // old creation
            LuckAttr.CreateValue(hexValue);
            AllignmentAttr.CreateValue(hexValue);
